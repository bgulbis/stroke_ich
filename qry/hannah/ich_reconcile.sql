...
SELECT DISTINCT
	...
	ORDER_RECON.PERFORMED_DT_TM,
	pi_from_gmt(ORDER_RECON.PERFORMED_DT_TM, 'America/Chicago') AS RECON_DATETIME,
	CASE ORDER_RECON.RECON_TYPE_FLAG WHEN 1 THEN 'Admit' WHEN 3 THEN 'Disch' END AS RECON_TYPE,
	CASE ORDER_RECON.NO_KNOWN_MEDS_IND WHEN 1 THEN 'TRUE' ELSE '' END AS NO_KNOWN_MEDS,
	ORDER_RECON_DETAIL.ORDER_MNEMONIC,
	ORDER_RECON_DETAIL.RECON_ORDER_ACTION_MEAN
FROM
	ENCNTR_ALIAS,
	INCLUDE_PTS,
	ORDER_RECON,
	ORDER_RECON_DETAIL
WHERE
	INCLUDE_PTS.ENCNTR_ID = ORDER_RECON.ENCNTR_ID
	AND ORDER_RECON.RECON_STATUS_CD = 861407102 -- Complete
	AND ORDER_RECON.ORDER_RECON_ID = ORDER_RECON_DETAIL.ORDER_RECON_ID
